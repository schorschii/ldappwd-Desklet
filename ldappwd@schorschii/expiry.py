...
if(len(sys.argv) != 6):
    print("Expecting 5 Parameters: expiry.py <Server URL> <Bind User> <Bind Password> <Search Base> <sAMAccountName>\n")
    sys.exit(1)
...
if(len(conn.entries) == 0):
    print("Error: User query produced no result entry")
    sys.exit(1)
if(len(conn.entries) > 1):
    print("Error: User query produced more than one result entry")
    sys.exit(1)
...
if(len(conn.entries) != 1):
    print("Error: maxPwdAge query produced no or more than one result entry")
    sys.exit(1)
